...
- **Ink**: a full-canvas Metal fluid-simulation inkwash layer (velocity/pressure/vorticity field with wet diffusion and Beer-Lambert density) with editable vector paths, paper grain, and fix/clear commands. The **pen** lays a smooth variable-width ribbon (capsule strokes unioned on the GPU — no beading); the **wash** brush injects a directional, accumulative smear into the velocity field, with a Strength slider and a hold-to-charge "heavy" mode, optionally destructive (re-mobilizes dried pigment). Adjustable stroke **smoothing**, **immediate** pen/wash modes (paint straight onto the canvas without recording an editable path), black/white ink, a **Rerender** button, and an Excalidraw-style mini editor. Frame-rate-independent and decoupled from the main thread for low-latency drawing. Shortcuts: `P`/`7` draw, `V`/`1` select, `A` points, `B` pen/wash, `W` black/white, `D` fix, `C` clear, Delete, undo/redo, save, fullscreen, and right-drag wash.
