...
- **Export**: a dedicated output-stream panel writes PNG/TIFF/JPEG/HEIF stills and sequences, GIF, H.264/HEVC, or ProRes movies without changing the live canvas. Capture cadence, playback rate, and NRT simulation rate are independent (`0.001…360 fps`). Sessions can run continuously or trigger from Capture Next, canvas mouse gestures, Draw/Wash phases, committed actions, solver activity, image change, motion, post-effect layers, or paper/motion control fields. NRT can replay timed gestures, continue an exact clone of the physical ink state, and use temporary Camera/Web proxies; movie sources can advance after each accepted frame for rotoscoping. Completed outputs can be scrubbed, cropped, rotated/flipped, re-exported, or continued non-destructively from an earlier frame. ⌘E exports the current frame with the configured still format; ⇧⌘E is the rebindable Capture Next action. See [`notes/output-stream-exporter.md`](notes/output-stream-exporter.md).
