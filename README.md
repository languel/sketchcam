...
- **Ink**: a full-canvas shader inkwash layer with editable vector paths, paper grain, Beer-Lambert ink density, wet diffusion, fix/clear commands, black/white ink, pen/brush modes, and an Excalidraw-style mini editor. Shortcuts include `P`/`7` draw, `V`/`1` select, `A` points, `B` pen/brush, `W` black/white, `D` fix, `C` clear, Delete, undo/redo, save, and fullscreen.
