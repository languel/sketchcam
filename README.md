...
- **Ink**: a full-canvas Metal fluid-simulation inkwash layer (velocity/pressure/vorticity field with wet diffusion and Beer-Lambert density) with editable vector paths, paper grain, and fix/clear commands. The **pen** lays a smooth variable-width ribbon (capsule strokes unioned on the GPU — no beading); the **wash** brush injects a directional, accumulative smear, with a single subtle→dramatic **Smear** dial. Independent **Pen size** and **Wash size** (type past 1 for bigger brushes). **Bleed** controls how watery vs acrylic the spread is (0 = push pigment without dissolving). **Color / Dissolve** ink (Dissolve = opaque white that covers/erases; a Dissolve wash clears to paper). A separate **Wash** colour (with opacity) tints the wet paper. **Fade** sets how long strokes settle after release and how long **Clear** dissolves out (the pigment fades while the paper stays). **Save-stroke** vs **immediate** per tool (immediate paints straight on the canvas without recording a path; the default). Editable parameter fields (type exact / out-of-range values), hover tips, double-click a label to reset, plus a **Rerender** button and an Excalidraw-style mini editor. Frame-rate-independent and decoupled from the main thread for low-latency drawing. Shortcuts: `P`/`7` draw, `V`/`1` select, `A` points, `B` pen/wash, `W` color/dissolve, `I`/`O` save-stroke pen/wash, `[`/`]` pen size, `⇧[`/`⇧]` wash size, `D` fix, `C` clear, Delete, undo/redo, save, fullscreen, and right-drag **or Ctrl-drag** to wash (all rebindable in the Keys tab). The side panel toggles beside the canvas (⌘⌥U, canvas resizes to fit) or as an overlay (⇧⌥U).
